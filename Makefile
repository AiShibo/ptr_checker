...
################### Set this flag if the function uses imsg ###################

# USE_IMSG: Skip imsg header (16 bytes) when checking for pointers
# imsg header is generated by library (not managed by applicaion), if check
# against msan, will be flaged as unitialized due to msan bugs on FreeBSD
#
# We also need to exempt the header from checking for pointers, due to a
# limitation in our library. We rely on exemption by md5 values, which
# we need the *entire* message to calculate its md5 value. However, the
# by the time we call imsg_compose, we only know the value of payload not
# the header! Therefore, the md5 is actually the md5 of the payload, and
# we need to skip the header in order to find match.
USE_IMSG = 1
...
# Build combined shared library (contains buffer_check and interception)
...
clean:
	rm -f $(COMBINED_LIB) $(MSG_GENERATOR_LIB) $(BUFFER_CHECK_OBJ) $(BUFFER_INTERCEPT_OBJ) $(MSG_GENERATOR_OBJ)
